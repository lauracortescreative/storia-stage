...
const StoryPlayer: React.FC<StoryPlayerProps> = ({ story, onClose, onSave, translations: t }) => {
  ...
  const handleTimeUpdate = () => {
    ...
    if (audio && audio.duration && isFinite(audio.duration)) {
      ...
      const fadeThreshold = 8; // Fade starts 8 seconds from the end
      ...
      if (story.sleep_fade && isLastEpisode && timeLeft < fadeThreshold && timeLeft > 0) {
        ...
        if (ambientRef.current) {
          ambientRef.current.volume = fadeFactor * 0.5;
        }
      } else {
    ...
    if (isPlaying) {
      ...
      audioRef.current.play().then(() => {
        ...
        if (ambientRef.current && story.soundscape !== 'none' && (ambientRef.current.src || story.soundscapeBlobUrl)) {
           if (!ambientRef.current.src && story.soundscapeBlobUrl) {
             ambientRef.current.src = story.soundscapeBlobUrl;
           }
           ambientRef.current.play().catch(e => {
             if (e.name !== 'AbortError') console.error("Ambient playback interrupted", e);
           });
        }
  ...
  const handleSave = () => {
    ...
    onSave();
    // Only show toast if it was successfully saved (parent state updates)
    // Note: onSave might trigger a view change to Auth if not logged in.
    // If we stay in player, it means they were logged in or just saved.
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center p-4 md:p-10 select-none overflow-hidden">
      {isGeneratingPdf && (
      ...
      <div className="w-full max-w-5xl flex flex-col gap-8 h-full">
        <div onClick={togglePlay} className="flex-1 relative rounded-[3rem] overflow-hidden bg-zinc-900 shadow-2xl border border-zinc-800 flex items-center justify-center group cursor-pointer">
          <div className="relative w-full h-full overflow-hidden">
            {(story.screen_mode === 'dark' || !subtitlesVisible) ? (
